...
fn part1() {
    ...
    for line in lines {
        let wire_operations = line.split("->").collect::<Vec<_>>();

        let wire_label = wire_operations.get(1).unwrap().trim().to_string();
        let insturctions = wire_operations
            .get(0)
            .unwrap()
            .trim()
            .split(" ")
        ...
        if insturctions.len() == 1 {
            ...
            if is_number {
                value = insturctions
                    ...
                    .unwrap()
                    .parse()
                    .expect("instructions with length 1 should be a number");
            } else {
                let wire1 = insturctions.get(0).unwrap().to_string();
                wire_instructions = Some(WireInstructions {
                    wire1,
                    instruction: "DIRECT".to_string(),
                    wire2: None,
                });
            }
        } else if insturctions.len() == 2
            && insturctions.get(0).unwrap().to_string() == "NOT".to_string()
        {
